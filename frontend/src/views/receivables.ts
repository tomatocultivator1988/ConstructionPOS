...
export async function renderReceivables(): Promise<string> {
  ...
  return `<div class="page-header"><div><div class="page-kicker">Credit sales</div><h2>Receivables</h2><p class="page-subtitle">Track customer balances and credit sales in one place.</p></div><button class="btn" onclick="exportReceivables()">Export</button></div>
...
export function exportReceivables() {
  showExportPeriodModal('Receivables', async (period: ExportPeriod, format) => {
    const result = await apiGet<any>(`/invoices/receivables?export=1&from=${period.from}&to=${period.to}&status=${receivableStatus}&search=${encodeURIComponent(receivableSearch)}`);
    const rows = (result.data || []).map((row: any) => [row.account_name, row.invoice_number, fmtDate(row.issued_date), fmtPeso(row.total), fmtPeso(row.net_paid), fmtPeso(Math.max(0, Number(row.balance || 0)))]);
    exportTable('Receivables', period, ['Customer / Account', 'Invoice', 'Sale Date', 'Total', 'Paid', 'Balance'], rows, format, `Outstanding balance: ${fmtPeso(result.summary?.outstanding || 0)}`);
  });
}
